...
文件名	：task_com2_gprs.c
模块名称：com2通讯任务 GPRS通讯
功能概要：com2通讯任务 GPRS通讯
...
//	uart2_set_bps(115200, 8, 1, NONE);
	//  com2_send_hex((void *)"12345", 5);
...
//  		form_comm_package(g_tmp_tbuf, &gprs.snd_len);
			...
			for ( j = 0; j < COMM_RETRY_TIMES; j++ )
			{
				...
				uart2_buf_flush();
				com2_send_hex(Com2SndBuf, gprs.snd_len);
				i = com2_gets(Com2RcvBuf, &gprs.rcv_len, 3000, 10);
				if ( i == ok )
				{
					break;
				}
...
//  		i = com2_gets(Com2RcvBuf, &gprs.rcv_len, 30000, 10);
//  		if ( i == ok && Com2RcvBuf[0] == 0x02 && Com2RcvBuf[2] == 0x00 )
//  		{
//  			//do nothing
//  		}
//  		else
...
//
//  		Com2SndBuf[0] = 0x06;
//  		Com2SndBuf[1] = Com2RcvBuf[1];
//  		com2_send_hex(Com2SndBuf, 2);
		...
		if ( gprs.update_time == TRUE )
		{
			...
			for ( j = 0; j < COMM_RETRY_TIMES; j++ )
			{
				...
				uart2_buf_flush();
				com2_send_hex(Com2SndBuf, gprs.snd_len);
				i = com2_gets(Com2RcvBuf, &gprs.rcv_len, 1000, 10);
				if ( i == ok )
				{
					break;
				}
			...
			i = com2_gets(Com2RcvBuf, &gprs.rcv_len, 10000, 10);
			if ( i == ok && Com2RcvBuf[0] == 0x02 )
			{
				//do nothing
			}
			else
			...
			dle_del(Com2RcvBuf, &gprs.rcv_len);

			if ( Com2RcvBuf[2] == 0x14 && Com2RcvBuf[3] == g_pack_num )
			{
				//do nothing
			}
			else
			...
			crc = Cal_Crc(&Com2RcvBuf[2], 9);
			if ( (Com2RcvBuf[2 + 9] == (INT8U)(crc >> 8)) && (Com2RcvBuf[2 + 9 + 1] == (INT8U)(crc & 0x00FF)) )
			{
				//do nothing
			}
			else
			...
			ltime.century = 0x20;
			ltime.year = Com2RcvBuf[5];
			ltime.month = Com2RcvBuf[6];
			ltime.day = Com2RcvBuf[7];
			ltime.hour = Com2RcvBuf[8];
			ltime.minute = Com2RcvBuf[9];
			ltime.second = Com2RcvBuf[10];
...
//  		if (rec.card_type == CARDT_DRIVER)
		...
		//  	if ( i == ok && Com2RcvBuf[0] == 0x02 && Com2RcvBuf[2] == 0x00 )
				...
				if ( Com2RcvBuf[2] == 0x11 && Com2RcvBuf[3] == g_pack_num && Com2RcvBuf[4] == 0x00 )
				{
					//do nothing
				}
				else
	...
	//  			htoa(disbuf, Com2RcvBuf, gprs.rcv_len);
	//  			lcddisp(0, 0, disbuf);
	//  			lcddisp(1, 0, "                ");
...
//  		//debug end
			...
			if ( Com2RcvBuf[2] == 0x12 && Com2RcvBuf[3] == g_pack_num )
			{
				//do nothing
			}
			else
			...
			crc = Cal_Crc(&Com2RcvBuf[2], 11);
			if ( (Com2RcvBuf[2 + 11] == (INT8U)(crc >> 8)) && (Com2RcvBuf[2 + 11 + 1] == (INT8U)(crc & 0x00FF)) )
			{
				//do nothing
			}
			else
			...
			memcpy(&gprs.black_name_version[0], &Com2RcvBuf[4], 7);
			gprs.black_num = (INT16U)Com2RcvBuf[11] * 0x100 + (INT16U)Com2RcvBuf[12];
...
//  						SleepMs2(20000);
//  						uart2_buf_flush();
//  						(void)com2_gets(Com2RcvBuf, &gprs.rcv_len, 1000, 10);
//  					}
						...
						if ( Com2RcvBuf[2] == 0x13 && Com2RcvBuf[3] == g_pack_num )
						{
							//do nothing
						}
						else
						...
						crc = Cal_Crc(&Com2RcvBuf[2], last_package_black_num * 4 + 2);
						if ( (Com2RcvBuf[2 + last_package_black_num * 4 + 2] == (INT8U)(crc >> 8)) 
							 && (Com2RcvBuf[2 + last_package_black_num * 4 + 2 + 1] == (INT8U)(crc & 0x00FF)) )
						{
							//do nothing
						}
						else
					...
					{
						black_rec[0] = Com2RcvBuf[4 + j * 4];
						black_rec[1] = Com2RcvBuf[4 + j * 4 + 1];
						black_rec[2] = Com2RcvBuf[4 + j * 4 + 2];
						black_rec[3] = Com2RcvBuf[4 + j * 4 + 3];
...
//  					}
						...
						crc = Cal_Crc(&Com2RcvBuf[2], BLACK_NUM_PER_PACK * 4 + 2);
						if ( (Com2RcvBuf[2 + BLACK_NUM_PER_PACK * 4 + 2] == (INT8U)(crc >> 8))
							 && (Com2RcvBuf[2 + BLACK_NUM_PER_PACK * 4 + 2 + 1] == (INT8U)(crc & 0x00FF)) )
						{
							//do nothing
						}
						else
...
void form_comm_package(INT8U *buf, INT16U *len)
{
	...
	memcpy(Com2SndBuf, buf, llen);

	if (g_turn_bit == 0x00)
	{
		Com2SndBuf[0] =( Com2SndBuf[0] & (~BIT0) );
		g_turn_bit = 0x01;
	}
	else
	{
	...
	{
		Com2SndBuf[0] =( Com2SndBuf[0] & (~BIT1) );
	}

	bcc = Cal_BCC(Com2SndBuf, llen);
	Com2SndBuf[llen] = bcc;
	llen++;

	dle_add(Com2SndBuf, &llen);

	memmove(&Com2SndBuf[1], &Com2SndBuf[0], llen);
	Com2SndBuf[0] = 0x02;
	Com2SndBuf[llen + 1] = 0x03;
	llen += 2;
...
{
	com2_send_hex((void *)snd_buf, (INT16U)strlen(snd_buf));
}
